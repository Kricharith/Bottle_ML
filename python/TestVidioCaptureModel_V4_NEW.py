...
def process_image(frame, image_path, model):
    ...
    cv2.putText(img, "Capture", (510, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
cap1 = cv2.VideoCapture(0)  # กล้องแรก
cap2 = cv2.VideoCapture(1)  # กล้องที่สอง
    ...
    for detection in results1:
        ...
        for box in boxes:
            ...
            if confidence > 0.7:
                ...
                cv2.putText(frame1, f"{category_name}: {confidence:.2f}", (int(x1), int(y1 - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
    for detection in results2:
        ...
        for box in boxes:
            ...
            if confidence > 0.7:
                ...
                cv2.putText(frame2, f"{category_name}: {confidence:.2f}", (int(x1), int(y1 - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
